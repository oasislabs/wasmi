...
use value::{
    ArithmeticOps, ExtendInto, Float, Integer, RuntimeValue, TransmuteInto, TryTruncateInto,
    WrapInto,
};
use Signature;
use Trap;
...
impl Interpreter {
    ...
    where
        T: FromRuntimeValueInternal + Copy,
    {
    ...
    where
        T: FromRuntimeValueInternal + WrapInto<U>,
        U: Copy,
    {
    ...
    fn run_grow_memory(
        ...
        self.value_stack.push(RuntimeValueInternal(u64::from(m)))?;
        Ok(InstructionOutcome::RunNextInstruction)
    }
    ...
    where
        T: FromRuntimeValueInternal + PartialEq<T> + Default,
    {
    ...
    where
        RuntimeValueInternal: From<U>,
        T: FromRuntimeValueInternal + TransmuteInto<U>,
    {
        let v = self.value_stack.pop_as::<T>();
...
pub fn check_function_args(signature: &Signature, args: &[RuntimeValue]) -> Result<(), Trap> {
    ...
    if signature
        ...
        .iter()
        .zip(args)
        .any(|(expected_type, param_value)| &param_value.value_type() != expected_type)
    {
        return Err(TrapKind::UnexpectedSignature.into());
    }
